...
FROM cards c
JOIN students s ON c.student_id = s.stdent_id  -- 👈 ใช้ stdent_id ตาม DB จริง
ORDER BY c.card_id
LIMIT 10;
"""
...
# 3) ร้านค้า
vendors_query = """
SELECT v.vendor_id, v.shop_name, SUM(t.amount) AS total_spent
FROM vendors v
LEFT JOIN transaction_logs t ON v.vendor_id = t.vendor_id
WHERE t.purpose='payment'
...
FROM transaction_logs
WHERE card_id = {selected_card}
GROUP BY DATE_TRUNC('month', txn_time)
ORDER BY month;
"""
card_data = pd.read_sql(card_query, conn)
